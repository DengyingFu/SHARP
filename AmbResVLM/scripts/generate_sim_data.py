...
"""Adapted from https://github.com/google-research/google-research/blob/master/language_model_uncertainty/KnowNo_TabletopSim.ipynb"""
...
class PickPlaceEnv:
            ...
            if ("block" in obj_name) or ("bowl" in obj_name):
                ...
                if object_type == "block":
                    ...
                    object_id = pybullet.loadURDF(
                        str(ASSETS_DIR.URDF / "bowl/bowl.urdf"), object_position, useFixedBase=1
                    )
    ...
    def get_camera_image_top(
        self,
        image_size=(240, 240),
        intrinsics=(2000.0, 0, 2000.0, 0, 2000.0, 2000.0, 0, 0, 1),
        position=(0, -0.5, 5),
    ...
    def render_image(
        self, image_size=(720, 720), intrinsics=(360.0, 0, 360.0, 0, 360.0, 360.0, 0, 0, 1)
    ):
...
def main():
    ...
    env = PickPlaceEnv()
    tasks = [
        "Pick the {block} and place it in the {bowl}.",
    ...
    for mode in ["train", "test"]:
        for _ in range(num_images):
            ...
            block_colors = random.sample(list(COLORS.keys()), 3)
            bowl_colors = random.sample(list(COLORS.keys()), 3)
            config = {
                "pick": [color + " block" for color in block_colors],
                "place": [color + " bowl" for color in bowl_colors],
            }
            _ = env.reset(config)
            img = env.get_camera_image()
            obj_list = config["pick"] + config["place"]
            ambiguity_map = {
                "block": [color + " block" for color in block_colors],
                "bowl": [color + " bowl" for color in bowl_colors],
            }
            id = shortuuid.uuid()
            out_dict = {
                "id": id,
            ...
            img_dir = DATA_DIR.get_dir("sim", mode)
            img_dir.mkdir(parents=True, exist_ok=True)
            pil_img.save(img_dir / f"{id}.jpeg")
            # Save to jsonl
            out_dir = ASSETS_DIR.DATA / "sim" / mode
            out_dir.mkdir(parents=True, exist_ok=True)
            out_file_path = out_dir / "data_raw.jsonl"
            with jsonlines.open(out_file_path, mode="a") as writer:

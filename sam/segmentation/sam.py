...
SAM_CHECKPOINT_PATH = os.path.normpath(
...
def get_mask(image, object_list, sam, detections, output_folder="output", selected_indices=None):
    sam_predictor = SamPredictor(sam)
    image = np.array(image)

    # filter detections if specific indices are provided
    ...
    # Use default color palette (matching mask colors) instead of forced black
    ...
    if output_folder.endswith(".jpg"):
        annotated_image.save(output_folder)
    else:
